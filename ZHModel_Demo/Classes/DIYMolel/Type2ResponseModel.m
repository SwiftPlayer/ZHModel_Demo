...
@implementation Type2ResponseSubInfoModel

@end


@implementation Type2ResponseModel
        ...
        if ([self respondsToSelector:@selector(setValue:forKey:)]) {
            if (class_getProperty([self class], [key UTF8String])) {
                if (![dic[key] isKindOfClass:[NSNull class]] ) {
                    if ([key isEqualToString:@"subInfo"]) {
                        self.subInfo = [[Type2ResponseSubInfoModel alloc] init];
                        [self.subInfo decodeJsontoDictionary:dic[key]];
                    }else{
